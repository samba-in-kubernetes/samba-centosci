    ...
    withCredentials([string(credentialsId: 'github-status-token', variable: 'GITHUB_TOKEN')]) {
        sh '''
            ...
            curl -s -X POST \
                ...
                -d "$json" \
                "https://api.github.com/repos/samba-in-kubernetes/sit-environment/statuses/${COMMIT_SHA}"
        '''
    }
}
...
def resolveTriggerContext() {
    if (env.WEBHOOK_PR_ACTION in ['opened', 'synchronize']) {
        ...
        withCredentials([string(credentialsId: 'github-status-token', variable: 'GITHUB_TOKEN')]) {
            def prData = sh(
                script: '''curl -s -H "Authorization: token ${GITHUB_TOKEN}" "https://api.github.com/repos/samba-in-kubernetes/sit-environment/pulls/${PULL_REQUEST_ID}"''',
                returnStdout: true
            ).trim()
...
def emailRecipients(String fileSystem) {
    def recipients = 'anoopcs@samba.org'
...
def buildCell(String fileSystem) {
    ...
    def cellName = "samba_${fileSystem}-integration-environment"
...
properties([
    ...
    parameters([
        string(name: 'BUILD_GIT_REPO', defaultValue: 'https://github.com/samba-in-kubernetes/sit-environment',
               description: 'Git repo URL of sit-environment (for manual triggers)'),
        string(name: 'BUILD_GIT_BRANCH', defaultValue: 'main',
               description: 'Branch of sit-environment to build from (for manual triggers)'),
    ...
    pipelineTriggers([
        ...
        [$class: 'GenericTrigger',
            ...
            token: 'samba-fs-integration-environment-trigger',
            tokenCredentialId: '',
